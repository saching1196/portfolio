...
loadSet = []  # list of tuples containing the load case type, magnitude of the load and location of the load

loadCases = int(input("Enter the total number of external load cases acting on the beam: "))
print("\n")

print("Enter 'CF' for point loads, 'UDL' for uniformly distributed loads, 'UVL' for uniformly varying loads, 'CL' for combined loads.")
...
for i in range(loadCases):
    ...
    elif loadCaseType == 'CL':
        startMagnitude = float(input("Enter the magnitude at the start of combined loading in N (+ve distribution upwards, -ve downwards): "))        
        clStart = float(input("Enter the horizontal distance in m to the start of CL from the left end of the beam: "))
        endMagnitude = float(input("Enter the magnitude at the end of combined loading in N (+ve distribution upwards, -ve downwards): "))
        clEnd = float(input("Enter the horizontal distance in m to the end of CL from the left end of the beam: "))
        loadSet.append([loadCaseType, startMagnitude, endMagnitude, clStart, clEnd])
    else:
...
for e in loadSet:
    if e[0] == 'UDL':
        ...
        udLoad = e[1] * (e[4] - e[3])
        uvLoad = 0.5 * (e[4] - e[3]) * (e[2] - e[1])
        sumOfForces += udLoad + uvLoad
        udMoment = udLoad * ((e[3] + e[4])/2)
        uvMoment = uvLoad * (e[3] + ( (2/3) * (e[4] - e[3]) ) )
        sumOfMoments += udMoment + uvMoment
    elif e[0] == 'UVL':
...
for i in range(len(loadSet)-1):
    if loadSet[i][0] == 'R':
        ...
        x = np.linspace(loadSet[i][2], loadSet[i][3], 1000, endpoint=True)       
        uvLoad = 0.5 * (x - loadSet[i][2]) * (((x - loadSet[i][2]) / (loadSet[i][3] - loadSet[i][2])) * loadSet[i][1])
        sumOfForces  += uvLoad
        shearForcesList.extend(sumOfForces)
        spanValuesList.extend(x)
        ...
        x = np.linspace(loadSet[i][3], loadSet[i][4], 1000, endpoint=True)
        udLoad = loadSet[i][1]*(x-loadSet[i][3])
        uvLoad = 0.5 * (x - loadSet[i][3]) * (((x - loadSet[i][3]) / (loadSet[i][4] - loadSet[i][3])) * (loadSet[i][2] - loadSet[i][1]) )
        sumOfForces += udLoad + uvLoad
        shearForcesList.extend(sumOfForces)
        spanValuesList.extend(x)
        
        x1 = np.linspace(loadSet[i][4], loadSet[i+1][2], 1000, endpoint=True)
        sumOfForces = sumOfForces[-1] * np.ones(1000)
        shearForcesList.extend(sumOfForces)
        spanValuesList.extend(x1)
...
for i in range(len(loadSet)-1):
    if loadSet[i][0] == 'R':
        ...
        for j in range(i):
            if(loadSet[j][0] == 'CL'):
                udLoad = loadSet[j][1] * (loadSet[j][4] - loadSet[j][3])
                uvLoad = 0.5 * (loadSet[j][4] - loadSet[j][3]) * (loadSet[j][2] - loadSet[j][1])
                udPointOfAction =  x - ((loadSet[j][3] + loadSet[j][4])/2) 
                uvPointOfAction =  x - (loadSet[j][3] + ( (2/3) * (loadSet[j][4] - loadSet[j][3]) ) ) 
                udMoment = udLoad * udPointOfAction
                uvMoment = uvLoad * uvPointOfAction
                sumOfMoments += udMoment + uvMoment
            else:
                sumOfMoments += loadSet[j][1] * (x - loadSet[j][2])
        
        udLoad = loadSet[i][1] * (x - loadSet[i][3])
        udMomentArm = x - ( (x + loadSet[i][3])/2 )
        baseOfUvLoad = x - loadSet[i][3]
        heightOfUvLoad = ( (x - loadSet[i][3]) / (loadSet[i][4] - loadSet[i][3]) ) * (loadSet[i][2] - loadSet[i][1])
        uvLoad = 0.5 * baseOfUvLoad * heightOfUvLoad
        uvMomentArm = x - (loadSet[i][3] + ( (2/3) * (loadSet[i][4] - loadSet[i][3]) ) )
        sumOfMoments  = sumOfMoments + (udLoad * udMomentArm) + (uvLoad * uvMomentArm)
        ...
        for j in range(i+1):
             if(loadSet[j][0] == 'CL'):
                udLoad = loadSet[j][1] * (loadSet[j][4] - loadSet[j][3])
                uvLoad = 0.5 * (loadSet[j][4] - loadSet[j][3]) * (loadSet[j][2] - loadSet[j][1])
                udPointOfAction =  x1 - ((loadSet[j][3] + loadSet[j][4])/2) 
                uvPointOfAction =  x1 - (loadSet[j][3] + ( (2/3) * (loadSet[j][4] - loadSet[j][3]) ) ) 
                udMoment = udLoad * udPointOfAction
                uvMoment = uvLoad * uvPointOfAction
                sumOfMoments += udMoment + uvMoment
             else:
...
#print(xValues)
formattedBendingMomentsList = [round(elem, 4) for elem in bendingMomentsList]
#print(formattedBendingMomentsList)
